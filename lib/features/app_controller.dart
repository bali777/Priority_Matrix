...
final taskRepositoryProvider = Provider<ITaskRepository>((ref) => HiveTaskRepository());
...
class AppController extends StateNotifier<AppState> {
  AppController(this._tasksRepo, this._settingsRepo)
      : super(const AppState(tasks: [], hideCompleted: false, inboxCollapsed: false));
  ...
  void init() {
    ...
    state = state.copyWith(tasks: _sortAll(tasks), hideCompleted: hide, inboxCollapsed: inboxCollapsed);
  ...
  List<Task> _sortAll(List<Task> tasks) {
    final copy = [...tasks];
    copy.sort((a, b) {
      if (a.listId.index != b.listId.index) return a.listId.index.compareTo(b.listId.index);
